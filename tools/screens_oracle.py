...
TRACE LAYOUT (little-endian) — FROZEN; a later task parses this exactly:
  ...
  Per RECORD (one screen-function call; carries BOTH entry and exit snapshots):
    ...
    u16   n_io           (# DAC OUT/IN events captured during this call window; nonzero
                          only for wait_vretrace_thunk / fns that call it)
...
OFF_PALETTE_MODE: int = 0x541d         # u16 (dispatch index for wait_vretrace_thunk)
...
# fns whose execution window we scope DAC port-I/O capture to. wait_vretrace_thunk
# (1000:9864) is a vsync wait dispatching via 2036:0000; on palette_mode==2 it emits
# 0 DAC events (vsync path). The genuine DAC writer is vga_dac_upload_from_buffer (0xb204).
# play_iris_wipe_transition (3467) progressively reprograms the DAC too.
...
def main() -> None:
    ...
    # wait_vretrace_thunk (0x9864) so the standalone palette-mode HANDLER's OWN DAC write
    ...
    def run_dac_seed_scenario() -> List[bytes]:
        ...
        wait_vretrace_thunk (0x9864), capturing the handler's own DAC writes."""
        cur_records.clear()
        capturing["on"] = True
        for mode in DAC_SEED_MODES:
            ...
            input_driver["active"] = False
            input_driver["queue"] = []
            call_near(0x9864, [])     # wait_vretrace_thunk
        capturing["on"] = False
        return list(cur_records)
    ...
    lines.append("Generated by `tools/screens_oracle.py`. Capture granularity = screen "
                 ...
                 "DAC OUT sequence (ports 0x3c8/0x3c9) scoped to wait_vretrace_thunk / "
                 "play_iris_wipe_transition windows.\n\n")
    ...
    lines.append("The screen fns load their image via open_resource(idx)/read_chunked "
                 ...
                 "palette at +0x33 that vga_dac_upload_from_buffer reads and pushes to "
                 "the DAC). This "
                 "makes the screen-build descriptors deterministic without re-running file "
                 "I/O. No bytes are injected by hand.\n" % SEED_HEADER_LEN)

    lines.append("\n## wait_vretrace_thunk / DAC dispatch\n\n")
    lines.append("`wait_vretrace_thunk` (1000:9864) is a vsync-wait thunk (NOT a DAC "
                 "writer); it dispatches into the FAR overlay `dispatch_by_palette_mode_2036` "
                 "(2036:0000), which indirect-calls the handler at "
                 "`DGROUP[palette_mode*2 + 0x6976]`. Under `palette_mode==2` (VGA, the "
                 "natural boot) that handler does NOT touch 0x3c8/0x3c9 — it is the vsync "
                 "wait path. The genuine DAC writer is `vga_dac_upload_from_buffer` "
                 "(image off 0xb204), which emits the canonical OUT 0x3c8/0x3c9 sequence. "
                 "We scope DAC OUT/IN capture (ports 0x3c8/0x3c9) to the "
                 "wait_vretrace_thunk AND play_iris_wipe_transition windows; "
                 "`play_iris_wipe_transition` (3467) calls wait_vretrace_thunk 4x/step "
                 "x10 steps during the rectangle wipe, so the iris-wipe record carries the "
                 "nested DAC sequence (the outer DAC window absorbs the nested calls).\n\n")
    lines.append("OBSERVED: under this boot `palette_mode == 2` throughout, and the "
                 "standalone `wait_vretrace_thunk` records carry 0 DAC events (the "
                 "palette_mode==2 dispatch handler does not touch 0x3c8/0x3c9). "
                 ...
                 "wait_vretrace_thunk — so the iris-wipe record carries the full "
