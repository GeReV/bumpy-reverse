...
| `game_stubs.c` | **Genuine carve-outs (Phase-9 T4; further reduced by the audio-subsystem branch, Phases A-E)** | NOT a reconstruction. Faithful-SIGNATURE no-op/benign-default stubs for the engine functions the reconstructed call graph references but that are deliberately NOT reconstructed — every remaining symbol is a documented CARVE-OUT, in the same spirit as the self-modifying graphics-overlay blitters and the L5 timer ISR. As of Phase-9 T4 the file held **58 carve-out function symbols + `mode_script_tbl`**; the audio-subsystem branch then reconstructed the sound L4/L6 hardware drivers + the 9 MIDI dispatch backends into `sound.c`/`midi.c` (see the Phase-6 sound-subsystem audit + the new MIDI-module audit below) and removed their stubs, so the file now holds **35 carve-out function symbols + `mode_script_tbl`** (`tools/validate_integration.sh`-confirmed), all in one of these classes: HARDWARE-INIT (the `init_game_session_state` CRTC/audio/resource/IRQ block), CRTC PAGE-FLIP (`present_frame`), int8-TIMING (`run_n_frames`/`rotate_timing_flags_and_wait`/`wait_keypress`), ENGINE STANDALONE LOADER (`load_current_level_data` — slice loads via `level.c start_level`), NEVER-DECOMPILED (`reset_round_counters` = `init_round_state` 0x31de; Ghidra-labelled but decompilation fails), RENDER-CORE LEAVES (`init_sprite_structs`, `init_fullscreen_view_desc`, `setup_fullscreen_view`, `apply_level_palette`, `show_text_screen`, `show_pause_screen`, `fun_75a2_poll_action`), OUT-OF-SCOPE LEAVES (the entity sweep `sweep_active_entities`/`FUN_1000_6183`, reached from `play_contact_sound`; the PIT hardware counter-init leaf `pit_set_counter0`, called from `sound.c`'s `timer_teardown_restore` and the unrelated, unreconstructed ISR-install subsystem; player handler-table targets; and the P2 indirect-call backend), and the `dos_abort` CRT thunk. Through Phases 2–8 and the audio-subsystem branch, the original deferral list was un-stubbed into real module bodies (player/player2/anim/screens/sound/midi/spawn/level); what remains is carve-out-only, and of that remainder only `sweep_active_entities`/`FUN_1000_6183` and `pit_set_counter0` are audio-adjacent (reached from the sound pipeline) — both are genuine out-of-scope carve-outs, not unfinished audio work: the entity sweep is a different subsystem (player contact-action handling) and `pit_set_counter0` is the PIT hardware-counter-reprogram primitive belonging to a separate, unreconstructed ISR-install subsystem, not the audio engine itself. **NAMING:** carve-out stubs use their Ghidra-canonical name (e.g. `sweep_active_entities` for 1000:6183) even though they are not reconstructed; the name documents what the binary calls the leaf, the stub body stays a no-op. (Through 2026-07-14 several of these — plus a handful of address-suffixed carve-out names elsewhere in `src/` — were still spelled with the raw `FUN_1000_<off>` Ghidra-decompiler default or a stale/mismatched address suffix; all were synced to their Ghidra-canonical names, and two Ghidra names that were themselves wrong (`9837`, `97a4`) were corrected first. See `docs/ghidra-symbol-map.md`.) The carve-out boundary (which `game_loop` callees are genuine hardware/CRT/int8-timing leaves vs reconstructed game logic) is ENFORCED by `tools/validate_integration.sh`: it links `BUMPY.EXE` (no dup symbols), asserts every `game_loop` per-tick callee resolves to its real module `.obj` (not `game_stubs.obj`), and asserts `game_stubs.obj`'s symbol set ⊆ the explicit carve-out allowlist; a built-in `--self-test` proves the gate fails when a spine callee regresses to a stub. |
...
| **L1 event wrappers** (T4): `play_action_sound` (63be), `play_contact_sound` (640c), `play_exit_sound` (6305), `play_pickup_sound` (645d), `play_event_sound_64c1` (64c1), `play_state_sound_647e` (647e) | Transcription | Each reads a per-device byte LUT (OPL table when `sound_device_state==4`, else std) indexed by an event/state global → sound id → `play_sound`. The six 0x30-byte LUTs (DGROUP 0x260e/0x263e/0x26ce/0x26fe/0x276e/0x278e) dumped byte-exact from the unpacked image. Semantic-state validated. |
| **L2 device state** (T4): `sound_select_device` (6de3), `snddrv_init` (88e5), `select_sound_device_from_mask` (891e), `snddrv_dispatch_a-d` (85b5/85db/8600/8626), `snd_busy_delay` (872e) | Transcription | The device init/select state machine (`sound_init_state` 0→1→2; `snddrv_mode` 0x85b3; `sound_active_device_mask` 0x5586). **CORRECTION (2026-07-13):** `snddrv_init` returns a **detected-device bitmask** which `sound_select_device` ANDs against the user-requested device — `\|4` = MPU-401 present, `\|1` = OPL2/AdLib present. An earlier reconstruction copied Ghidra's `substep_ok`-always-`true` decompile and treated the two `if(!substep_ok){status\|=…}` arms as dead → hard-coded `status=0`, which zeroed the mask for **every** device (`req & 0 == 0`) → AdLib never reached `snddrv_mode=1` (OPL emit) → the title/intro MIDI was silent. The arms are NOT dead: the raw asm (88e5) `JZ`s on the **zero flag left by each sub-call's return** (`mpu401_reset_to_uart` → `\|4`, `snddrv_init_substep` → `\|1`), which Ghidra could not model and rendered as a never-reassigned local. Now reconstructed as `if(mpu401_reset_to_uart()!=0) status\|=4; if(snddrv_init_substep()!=0) status\|=1;`. Host `validate_sound` unaffected (host detects neither device → `status=0`, and the captured `device_init` entry `sound_device_state=0` masks it either way; PASS=4414 before and after); on dosbox-x the emulated OPL2 answers the timer probe → an AdLib request selects `snddrv_mode=1`. Semantic-state validated. |
...
| **L5 ISR tone-sequencer** (T6): `pit_timer_isr_multiplexer` (7c02), `tone_seq_callback_9631` (9631), `tone_seq_callback_96c4` (96c4), `tone_seq_callback_95b5` (95b5) | **Behavior-faithful; runtime-wired in the playable host (2026-07-13)** | The PIT (IRQ0/int-8) multiplexer walks the **0x549c timer-SLOT table** each tick (`accumulator[+2] += increment[+0]`; on the 500-tick period far-call the slot's installed callback `{cb_seg@+4, cb_off@+6}`) and the three tone-sequencer callbacks advance the L3 param frame + reprogram PIT ch2 (0x42/0x43) / strobe the speaker gate (0x61) to sweep the tone. **TABLE-WIRING FIX (2026-07-13):** the earlier reconstruction wrongly walked `snd_timer_cb_table` (0x5516) and read cb_off/cb_seg at +4/+6; verified vs raw disasm (7c02 `lds bx,[0x54cc]` → the 0x549c slot table; 7e62 writer; 7cde install), the asm mux walks the **0x549c** table — the one the L3 schedulers / `set_timer_slot_raw` install the tone callbacks into — NOT 0x5516 (a separate `arm_timer_callback` table serviced by the inert 0x7e80 sub-sweep). The old mux read the wrong table and would have dispatched nothing. Reached in the original ONLY via the installed far pointer — NO Ghidra function boundary, NOT hooked by the oracle (ZERO trace records, no host differential); validated by inspection vs the asm. **HOST INTEGRATION (2026-07-13):** split into `snd_timer_slot_sweep()` (the no-EOI 6-slot walk + two-pass store-then-dispatch, matching the asm trampoline order) + a thin `pit_timer_isr_multiplexer` wrapper (sweep + EOI) kept as documentation; the playable host `host_int8_isr` (host/host_timer.c) now calls the sweep every PIT tick so scheduled SFX tones actually advance and gate off — **fixes the playable-build "high-pitched noise during gameplay" (PC-speaker SFX playtest-confirmed 2026-07-13).** The default build is unchanged at runtime (the mux is still never called there). |
| **MT-32 / PC-speaker device completeness** (2026-07-13, playable playtest follow-up): `mpu401_present`/`midi_seq_step_active` static inits, `snd_busy_delay` (872e) MPU raw-forward, `pcspk_music_render` (9136) + `pcspk_set_pit_ch2` (91b9) + `pcspk_io_settle` (8a3b) + `pcspk_freq_table` (0x8489) | **Transcription** (device inits, MPU forward) / **Behavior-faithful, installed-cb** (`pcspk_music_render`, no Ghidra boundary) | Three fixes after AdLib was confirmed working. **(1) MT-32 device select:** `mpu401_present` (0x557c) and `midi_seq_step_active` (0x557e) are statically `0x0001` in the image but were BSS-0 → `mpu401_reset_to_uart` early-returned 0 → `snddrv_init` never set the MPU status bit (`\|4`) → MT-32 (mask 4) never reached `snddrv_mode=4`. Faithful values (=1) restored (DGROUP layout is pinned but *values* come from per-global C initialisers, not an image blob). **(2) MT-32 emit:** `snd_busy_delay` is misnamed — it is the MPU raw-MIDI forwarder (mode-4 dispatch → `snd_busy_delay` → `mpu401_write_data_polled` → OUT 0x330). The prior body called the writer without staging the byte (`snd_mpu_byte_89e2`) or `LODSB`-advancing `snd_seq_cursor`, so mode-4 forwarded stale bytes AND misaligned the MIDI cursor; now sends the ambient status byte then forwards `count` live stream bytes (asm 872e verbatim); `snddrv_dispatch_d_mode4`/`_b_mode4` stage AL into `snd_seq_event_al` at the call. **(3) PC-speaker music:** `select_sound_device_from_mask` already installed cb 0x9136 into 0x549c slot ch1 (`set_timer_slot_raw(1,0x64,0x9136,0x1000)`) but the renderer was never reconstructed, so the sweep dispatched nothing → PC BASE was silent. `pcspk_music_render` (reconstructed from raw disasm 9136 — installed far-pointer cb, no Ghidra boundary) is a 2-voice round-robin that reads `snd_voice_table` (filled by `snddrv_dispatch_d_mode0`), looks up a PIT divisor in `pcspk_freq_table` (0x8489, dumped verbatim), programs PIT ch2 via `pcspk_set_pit_ch2` (91b9) + gates 0x61; wired into `snd_timer_slot_sweep` alongside the tempo tick. `pcspk_transpose_map` (0x83f1, all-zero in image) modelled as a separate zero array (CS-adjacency to the freq table not bit-exact for program bytes > 0x97 — immaterial). All gates green after (sound 4414/midi 334/int8/integration/screens); AWAITS PC-BASE + MT-32 playtest. |
| **MIDI dispatch backends** (T7 closeout): `snddrv_dispatch_b_mode0/1/4` (91cf/8e48/8af6), `snddrv_dispatch_c_mode0/1/4` (91d7/8e50/8b04), `snddrv_dispatch_d_mode0/1/4` (91df/8b0d/8e58) | **Behavior-faithful (NOT oracle-exercised)** | The last 9 stub gaps `snddrv_dispatch_b/c/d` (already-ported) fan out to. Verified via the RAW disassembly (the Ghidra decompiler's simplified view hides the real body for 6 of the 9 — e.g. `snddrv_dispatch_b_mode0` decompiles to a bare `return in_AX` even though the asm does `LODSB;ADD SI,AX`, because the decompiler doesn't track SI as an output): all 9 read/advance the caller's ambient DS:SI (a live MIDI-track byte cursor), AL (the MIDI event status byte), and/or CS:[BX+0x80] (a per-track default-channel byte) as raw REGISTERS, not stack args. Confirmed via Ghidra xrefs: their ONLY real caller is `midi_process_event` (8751/8756/875b), itself register-entry (`unaff_SI`) and — AT THE TIME OF THIS TASK — genuinely unreconstructed, separate, not-yet-started MIDI-engine work (this task closes the sound-**effect** pipeline's last gaps "before the MIDI engine work begins," per the task brief). **UPDATE (Task E2):** `midi_process_event` and the full MIDI/SMF engine are now reconstructed 1:1 in `src/midi.c` — see this table's later MIDI rows (MPU reset / OPL2 driver leaves / MIDI-to-OPL2 voice bridge) + the Phase-6 status section's Task E2 closeout below. Reconstructed 1:1 from the raw asm; register-entry inputs modelled as file-scope globals (`snd_seq_event_al`/`snd_seq_cursor`/`snd_seq_default_chan`, the `snd_mpu_byte_89e2` convention) since the true caller supplying them is out of scope and the EXISTING `snddrv_dispatch_b/c/d` call sites (unchanged by this task) pass no args. `snd_seq_cursor` defaults to a zero-filled scratch buffer (not real captured MIDI data — none exists) purely so the incidental invocation from the already-validated `snddrv_dispatch_b/c/d` mode0 replay (which DOES happen — `t4_l2_dispatch` seeds mode 0) is memory-safe; never inspected/asserted by any comparator. 3 further NEW carve-out leaves discovered and stubbed (`seq_set_channel_param` 922c, `midi_emit_voice_msg_w3` 8e93, `opl_event_note_on` 8ea3 — already canonically named, none reconstructed, same class as `FUN_1000_6183`/`FUN_1000_7fef`). |
| **MPU reset / init substep / timer teardown / status latch** (Task A3): `mpu401_reset_to_uart` (8a75), `snddrv_init_substep` (8b2a), `timer_teardown_restore` (7fef), `record_min_status_code` (945b) | **Transcription** (`mpu401_reset_to_uart`/`record_min_status_code`, oracle-inert) / **Behavior-faithful** (`snddrv_init_substep`'s ZF branch is immaterial **to the host gate** — see the correction below; `timer_teardown_restore`'s guarded body never executes) | Finishes the last 4 stubbed sound-pipeline leaves. **NB (2026-07-13):** `snddrv_init_substep`'s ZF verdict is immaterial to `validate_sound` (the host detects no OPL2 → returns 0, and the captured `device_init` entry masks the result) but is NOT immaterial at runtime — it is `snddrv_init`'s OPL-detection bit (`\|1`), which on real/emulated AdLib hardware selects `snddrv_mode=1`; see the **L2 device state** row's CORRECTION. `mpu401_reset_to_uart`: MPU-401 chip reset → UART mode, gated on `mpu401_present` (**CORRECTED 2026-07-13**: statically initialised to 1 in the image at DGROUP 0x557c, not BSS-0 — see the **MT-32 / PC-speaker device completeness** row; on the host it still runs to a DSR-timeout that clears the flag, so no oracle port-write record and `validate_sound` is unchanged). `record_min_status_code`: latches AX into `last_status_code` (0x946c) — see deviation (i). `snddrv_init_substep` (renamed from `FUN_1000_8b2a`, Ghidra's own canonical name): sets `midi_seq_step_active`, calls the carved-out `maybe_opl2_detect_chip` (8fb6) then branches on its ZF-only verdict to clear the flag or call the equally carved-out `opl2_reset_all_regs` (8eeb) — see deviation (i). `timer_teardown_restore` (renamed from `FUN_1000_7fef`): register-entry (AX=table index, CX/DX=vector-8 restore far ptr — confirmed no `[BP+n]` stack access in the disasm), guarded by `isr_installed_flag` (0x54d4, defaults 0 and never set nonzero by any reconstructed code — the guarded body, including 3 DOS INT 21h vector get/sets via `int86x`/`union REGS`/`struct SREGS` and the carved-out `pit_set_counter0` (7f9a), is **provably unreached** by every scenario in the trace). All 4 wired into their ALREADY-VALIDATED call sites (`snddrv_init` calls the first two; `timer_restore` calls the third; 6 already-committed L3 fns call the fourth) with **zero effect** on any of those records — confirmed by re-running `validate_sound`: identical PASS=4414/FAIL=0/UNPORTED=25/PORT_CHECKED=3752 before and after. |
...
- **(d) `play_state_sound` gameplay records UNPORTED.** `play_state_sound_647e` (647e) tail-
...
- **(h) MIDI dispatch backends: register-entry, not oracle-exercised (closeout task).**
  ...
  `midi_process_event`, is itself register-entry and — AT THE TIME OF THIS TASK (A1+A2) —
  genuinely unreconstructed, separate, not-yet-started MIDI-engine work (see the
  fidelity-table row above for the full reasoning; **now reconstructed, Task E2** — see
  that row's update note). The combined task brief that closed these out (plan tasks A1+A2) called for
...
**Closeout (Task E2 — sequencer driver + tempo timer: `midi_process_event` 873c,
`midi_load_sequence` 87cd, `midi_start_playback` 8722, `midi_sound_init` 89a8,
`midi_play_sequence` 8977, `midi_install_tempo_timer` 86e9):** the final 6 `src/midi.c`
bodies land, completing the MIDI module. `midi_process_event` — the centerpiece — is the
per-track event-stream cursor `midi_init_track_table` calls conditionally (Task E1); its
reconstruction unblocks `midi_parse_file`'s and `midi_init_track_table`'s own differential
(both real bodies since Task E1, left UNPORTED pending this fn) — both now PASS
(`check_tbl=1`, their contract IS the ptr/time table install).
- **(u) `midi_process_event`'s dispatch wiring, verified against the raw disasm (not the
  decompiler's confused SSA merge).** `decompile_function_by_address` renders this fn's
  loop-continuation test as a single reused `uVar5` boolean threaded through unrelated
  branches (the running-status check, the 0xf0/0xf7 compares, the meta-type compares) —
  a decompiler SSA artifact, not a real shared condition. The RAW disassembly
  (1000:873c..87a1) shows the loop actually works as: LODSB the next status byte; if its
  high bit is clear (no running-status support in this engine) eat one more byte and
  fall through; 0x80..0xEF (channel-voice) stages `snd_seq_event_al` and calls
  `snddrv_dispatch_d`; 0xF0 calls `snddrv_dispatch_c`; 0xF7 calls `snddrv_dispatch_b`
  (all three already-PORTED sound.c backends); otherwise a META event: LODSW the
  `{type,len}` pair RAW (no byte-swap, unlike the big-endian MThd/MTrk header fields
  `midi_parse_file` reads via `midi_bswap16`) and branch on type — 0x51 (Set Tempo) stores
  the 24-bit tempo split hi/lo; 0x2F (End of Track) consumes 2 more bytes, decrements
  `midi_track_count`, and returns `0xFFFFFFFF` IMMEDIATELY (skipping the shared tail
  entirely — no `midi_read_varlen` call, no `seq_normalize_far_ptr`); 0x20 (MIDI Channel
  Prefix) stores the channel byte into `CS:[BX+0x80]` — the ACTIVE track's slot in the
  per-track `midi_track_chan_table` (2026-07-13: `BX`/track index is now modelled as
  `midi_current_track`, set by the two `midi_process_event` callers; the earlier
  single-`snd_seq_default_chan` flat-global collapsed all tracks' prefixes and let
  interleaved tracks clobber each other's channel on the MPU/OPL path). The value is
  mirrored into `snd_seq_default_chan` (the flat-global the 9 `snddrv_dispatch_b/c/d`
  modeX backends still READ, so those validated leaves are untouched), and each caller
  refreshes `snd_seq_default_chan` from the per-track table before dispatching that
  track's events. Any other meta type skips its `len` data bytes. The shared tail (reached by every branch except EOT) decodes the next
  delta via `midi_read_varlen`; if it's 0, loop back and process another due event
  immediately (asm label 873d, NOT the function's own entry 873c — so internal
  iterations are invisible to the Task C2 oracle's function-boundary hook: 14 total
  records for a 7-track file across 2 full cascades = exactly one record per track per
  cascade, however many events each processed internally); otherwise
  `seq_normalize_far_ptr()` and return the delta. `BX` is confirmed genuinely ambient
  (no `MOV BX,...` anywhere in the fn) via `disassemble_function`; the Task C2 capture's
  own `bx` register field climbs by 4 per record (0x81cc, 0x81d0, ...), confirming
  `midi_init_track_table` — not this fn — supplies it as `track_ptr_table_base +
  idx*4`, exactly as documented.
- **(v) `midi_load_sequence`'s song_data/aux_ptr NAME CAVEAT (verified via raw disasm,
  independently corroborated by `tools/midi_oracle.py`'s own pre-existing header
  comment, lines 29-38).** Despite the established prototype's parameter names
  (`song_data` 1st, `aux_ptr` 2nd, matching the brief + Ghidra's own decompiled
  signature), the asm's actual data flow is the OPPOSITE: `song_data` (1st param, BP+4,
  loaded via `LDS SI,[BP+4]`) is stored into `midi_aux_ptr_off`/`_seg` AND — since that
  LDS is never reloaded before the `CALL 8809` — is ALSO the DS:SI "file image" argument
  `midi_parse_file`'s register-entry ABI reads (confirmed: its captured entry snap shows
  DS:SI pointing exactly at the real Bumpy.mid bytes fed through this param); `aux_ptr`
  (2nd param, BP+8, `LES SI,[BP+8]`) is stored into `midi_song_data_off`/`_seg` — the
  table `midi_emit_voice_msg_w1` (Task D2) walks for per-channel instrument lookups.
  Same "Task C1 globals-only skeleton speculatively named a global before any body
  reconstruction confirmed its real data flow" pattern as the pre-existing
  `midi_data_seg` a.k.a. "midi_load_flag" caveat — NOT renamed (would ripple into the
  already-validated Task D2 `w1` body + the FROZEN MIDI_SNAP trace field names +
  `tools/midi_ctest.c`); documented at the fn's own definition instead.
- **(w) `midi_install_tempo_timer` — tempo-ISR CARVE-OUT (the Phase-6 L5 precedent).**
  The fn's own body (compute the PIT reload value from `midi_division` × 0xf42 divided
  by the 24-bit tempo, then `set_timer_slot_raw(0, quotient, 0x864c, 0x1000)`) IS
  reconstructed 1:1 and DOES run for real on every replay that reaches it (transitively,
  via `midi_start_playback`, a PORTED entry) — its 3 captured entry states all have a
  verified non-zero divisor (0x0c35/0x0c35/0x07a1), so this is safe, not a host-side
  guard. What is NOT host-replayable is the per-PIT-tick PLAYBACK LOOP this installs:
  `set_timer_slot_raw` only writes the DGROUP timer-slot table (confirmed port-I/O-free
  — the oracle opened an L4 port-capture window for this fn's own address anyway, per
  its `L4_FNS` set, and captured ZERO OUT/IN events across all 3 records, empirically
  confirming the task brief's "0 port I/O" claim); the real sequencer advance lives in
  the L5 ISR machinery (`pit_timer_isr_multiplexer`/`tone_seq_callback_*`, sound.c),
  reached only through the far-pointer timer-slot install, driven by hardware IRQ0
  ticks a deterministic host replay cannot reproduce. Registered UNPORTED-for-validation
  (no `tools/midi_ctest.c` `PORTED[]` entry) — its own reload-value computation has
  NOTHING observable in the MIDI_SNAP format to assert against
  (`snd_timer_slot_table` isn't a captured field), so even a trivial "0 OUT events"
  entry would assert nothing beyond what this note documents; not fabricating a PIT
  port event this fn doesn't emit.
- **(x) `midi_play_sequence` has ZERO trace records — an empirical correction to the task
  brief's assumed "1 record".** Root-caused in the Task C2 oracle's own capture
  mechanism: `midi_play_sequence`'s real asm is a TAIL JMP (not CALL) into
  `midi_load_sequence`, reusing the caller's stack frame with NO new return address
  pushed; the oracle's function-boundary hook tracks pending exits per return address
  in a LIFO list, so when execution reaches `midi_load_sequence`'s entry (itself
  hooked), a SECOND entry gets pushed onto the SAME return-address's pending list; the
  single `RET` back to that shared landing pad pops only the LIFO-topmost entry
  (`midi_load_sequence`'s), silently orphaning `midi_play_sequence`'s own pending entry
  forever (verified by parsing the actual trace: `0x8977` never appears in the
  per-`fn_off` record tally). Registered in `PORTED[]` anyway (reconstructed 1:1,
  matching the task's explicit ask) but genuinely never exercised by
  `run_per_function` — documented, not fabricated.
- **(y) `set_timer_slot_reg` (1000:7e1f) — reused across translation units, renamed to
  match Ghidra.** `midi_sound_init`/`midi_play_sequence` both call this SAME physical
  address (`MOV AX,0x0; CALL 0x1000:7e1f`, confirmed via raw disasm of both) that
  Task A3 already reconstructed in `src/sound.c` under the LOCAL name
  `isr_disable_timer_slot` with `static` linkage (only its own 2 L5 tone-sequencer
  retire-path callers). Reusing the ALREADY-reconstructed body (not duplicating it,
  per "never invent") requires exposing it across TUs; renamed to Ghidra's own
  canonical label (`get_function_by_address`/`decompile_function_by_address` confirm
  1000:7e1f's canonical name IS `set_timer_slot_reg`, register-entry `in_AX`=channel,
  matching the fn's own `channel` param) and un-`static`-ed + prototyped in `sound.h`.
  Pure rename + linkage change, zero behavior difference; `sound_ctest.c` never
  referenced the old name (grep-verified), so `validate_sound`'s pre-existing baseline
  (PASS=4414 FAIL=0 UNPORTED=25 PORT_CHECKED=3752) is unaffected.
- **(z) Host-harness fix: `midi_track_ptr_table`'s check_tbl comparator now compares by
  LINEAR ADDRESS, not raw bytes.** Wiring `midi_parse_file`/`midi_init_track_table`'s
  `check_tbl=1` gate for the first time (Task E1 left both UNPORTED, so this 128B
  compare was never previously exercised) surfaced a REAL divergence: 2 of the
  captured trace's 16 track slots (tracks 0 and 6) show `{off,seg}` bytes that differ
  from this host's own recomputed split while resolving to the IDENTICAL linear address
  (`0x8002b` / `0x88947` respectively — verified by hand). Root cause: a far
  segment:offset pair is NOT a unique encoding of a linear address, and the engine's
  OWN un-normalized far-pointer arithmetic (Watcom `-ml`: `snd_seq_cursor` increments
  only the offset half; `seq_normalize_far_ptr` is the engine's explicit, occasional
  "roll excess offset into segment" step — NOT run after every byte) can leave the
  real offset >15 between normalize calls (e.g. a few bytes of MTrk-header validation
  consumed right after a normalize that already reduced the accumulated offset to
  0-15). The host's merged single-pointer `snd_seq_cursor` model (established for
  `seq_normalize_far_ptr` itself, unaffected here — see (n) above) always yields the
  SAME linear address as the real DS:SI pair at every point, but the host's
  `FP_SEG`/`FP_OFF` shim (`tools/midi_ctest.c`'s `((linear)>>4)`/`(linear&0xF)`,
  ALWAYS-fully-renormalized split) is not guaranteed to reproduce the SAME split the
  real, less-eagerly-normalized far pointer happened to have. Fixed the COMPARATOR
  (not the reconstructed C, which is correct) to check `(seg<<4)+off` equality per
  track slot rather than `memcmp`; this is the engine's own actual contract for this
  table ("a valid pointer to the track's position", not a specific bit-for-bit split)
  — a genuine cursor-advance bug would still diverge here (a wrong byte position
  changes the linear address, not just its split). `midi_track_time_table` (the
  decoded delta values, unaffected by pointer-split ambiguity) is still `memcmp`'d
  byte-exact, unchanged.

Gate: `validate_midi` FAIL=0, 334 PASS (220 PORT_CHECKED) / 3 UNPORTED (all
`midi_install_tempo_timer` — the documented tempo-ISR carve-out, (w) above) out of 337
total records — ALL 5 of this task's PORTED targets (`midi_process_event`,
`midi_load_sequence`, `midi_parse_file`, `midi_init_track_table`, `midi_start_playback`,
`midi_sound_init`; `midi_play_sequence` registered but genuinely 0 records, (x) above)
moved UNPORTED -> PASS. `validate_sound` unaffected: still PASS=4414 FAIL=0 UNPORTED=25
PORT_CHECKED=3752 (the `set_timer_slot_reg` rename is behavior-neutral, (y) above).
`validate_integration`: PASS (no dup symbols, all 39 spine callees resolve;
`midi_process_event` removed from `game_stubs.c`'s `CARVEOUT_ALLOWLIST` — it's a real
body now, not a stub).

**Deferred from Phase 6 — NOW CLOSED (Task E2).** The sound-**effect** pipeline
...
ends (play_intro_animation_loop, screens.c, after midi_sound_init tears the sequencer down) rather
than kept for the whole run.  WHY THIS MATTERS: the host also allocates hv_saveunder_buf (32 KB,
Task 7) that the engine never does; holding the SMF's 36 KB on top of it overflowed the reduced
640 KB far heap so hv_saveunder_buf's level-load _fmalloc(32000) returned NULL (verified live under
dosbox-x: with the SMF held a 32000-byte _fmalloc fails; after _ffree(SMF) it succeeds), which
NULLed host_clean_bg() and disabled the clean-bg erase/save-under paths → platform trails, items
not erased on pickup, and the EGA top-strip flag flicker.  Freeing after the intro reproduces the
engine's transient-SMF behaviour within the host's tighter heap.
